...
@Repository
public class AdminCompanyInfoDaoImpl extends BaseDao<CompanyInfo> implements IAdminCompanyInfoDao {
    @Override
    public CompanyInfo updateCompanyInfo(CompanyInfo companyInfo) {
        String sql = "UPDATE h_company_info SET company_address_CN=?,company_address_HK=?,company_address_US=?,company_domain=?, " +
    ...
    public CompanyInfo selectCompanyInfo() {
        String sql = "select * FROM h_company_info";

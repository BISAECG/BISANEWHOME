<!DOCTYPE html>
<html>
<head>
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="initial-scale=1.0,user-scalable=no">
    <link rel="icon" href="Images/favicon.ico" type="image/x-icon" />
    <link rel="shortcut icon" href="Images/favicon.ico" type="image/x-icon" />
    <link rel="bookmark" href="Images/favicon.ico" type="image/x-icon" />
    <meta charset="UTF-8">
    <meta name="keywords" content="${html_keyWord}">
    <meta name="description" content="${html_description}">
    <title>${html_title}</title>
    <link href="../resources/css/comm/swiper.min.css" rel="stylesheet">
    <link href="../resources/css/comm/base.css" rel="stylesheet">
    <link href="../resources/css/index/index.css" rel="stylesheet">
    <script src="../resources/js/comm/jquery.min.js"></script>
</head>
<body>
<div id="header"></div>
<script>
    $("#header").load("header.html");
</script>
<div class="con">
    <div class="container-fluid">
        <div class="row ">
            <img  class="img-responsive center-block" src="../resources/img/solution/banner6.png" alt="碧沙心电仪合作伙伴">
            <!--<img class="img-responsive center-block" src="../resources/img/index/banner6.png" alt="碧沙心电仪合作伙伴">-->
        </div>
    </div>
    <p class="title-p">为什么选择悉心动态心电记录仪?</p>
    <div class="line">
        <span class="line-top" ></span>
        <span class="line-btm" ></span>
    </div>
    <!--心电仪部分-->
    <div class="container-fluid mt-20 mb-20">
        <div class="row  pos-r" >
            <img  class="img-responsive center-block" src="../resources/img/index/banner7.png">
            <img  src="../resources/img/index/banner7-1.png" alt="检测到停搏自动通知联系人" class="pos-a " style="left: 25%;top: 42%;">
            <img src="../resources/img/index/banner7-2.png" alt="双击app按钮通知联系人" class="pos-a " style="left: 20%;top: 63%;">
            <img src="../resources/img/index/banner7-3.png" alt="双击操作键通知联系人" class="pos-a" style="left: 25%;top: 85%;">
            <img src="../resources/img/index/banner7-4.png" alt="一对一电话咨询认证医师" class="pos-a " style="right: 26%;top: 42%;">
            <img src="../resources/img/index/banner7-5.png" alt="15分钟免费检测报告服务" class="pos-a " style="right: 20%;top: 63%;">
            <img src="../resources/img/index/banner7-6.png" alt="专科认证医师在线答疑" class="pos-a " style="right: 25%;top: 85%;">
        </div>
    </div>
    <p class="title-p">产品证书</p>
    <div class="line">
        <span class="line-top"></span>
        <span class="line-btm"></span>
    </div>
    <!--产品证书展示部分-->
    <div class="container-fluid " style="background:url('../resources/img/index/banner1.png') round;">
        <div class="row">
            <div class="col-md-2 col-md-offset-1 mt-30 chanpin-pic">
                <a href="../resources/img/index/chanpin.png" target="_blank">
                    <img  src="../resources/img/index/chanpin.png" alt="医疗器械生产许可证">
                    <p>医疗器械生产许可证</p>
                </a>
            </div>
            <div class="col-md-2 mt-30 chanpin-pic">
                <a href="../resources/img/certificate/zcz.png" target="_blank">
                    <img  src="../resources/img/index/chanpin1.png" alt="CFDA注册证">
                    <p>CFDA注册证</p>
                </a>
            </div>
            <div class="col-md-2 mt-30 chanpin-pic">
                <a href="../resources/img/certificate/CE@2x.png" target="_blank">
                    <img src="../resources/img/index/chanpin2.png" alt="FCC认证">
                    <p>FCC认证</p>
                </a>
            </div>
            <div class="col-md-2 mt-30 chanpin-pic">
                <a  href="../resources/img/certificate/FCC@2x.png" target="_blank">
                    <img  src="../resources/img/index/chanpin3.png" alt="蓝牙">
                    <p>蓝牙</p>
                </a>
            </div>
            <div class="col-md-2 mt-30 chanpin-pic">
                <a  href="../resources/img/certificate/QBQ@2x.png" target="_blank">
                    <img  src="../resources/img/index/chanpin4.png" alt="FCC蓝牙QBQ认证">
                    <p>FCC蓝牙QBQ认证</p>
                </a>
            </div>
        </div>
    </div>
    <p class="title-p">视屏展示</p>
    <div class="line">
        <span class="line-top"></span>
        <span class="line-btm"></span>
    </div>
    <!--视屏展示部分-->
    <div class="container">
        <div class="row">
            <div class="col-md-6 col-md-offset-1 " style="background: red;height:372px;">
            </div>
            <div class="col-md-4 col-md-offset-1 mt-40 " style="background: #2B2B31;padding: 0;">
            </div>
        </div>
    </div>
    <!--公司介绍部分-->
    <div class="container-fluid  mt-20" >
        <div class="row">
            <img  class="img-responsive center-block" src="../resources/img/index/banner2-cn.png">
        </div>
    <#--<div class="row">-->
    <#--<div class="col-md-6 col-md-offset-3 text-center">-->
    <#--<p class="new-first">碧沙科技(香港)有限公司></p>-->
    <#--</div>-->
    <#--</div>-->
    <#--<div class="row">-->
    <#--<div class="col-md-6 col-md-offset-3 text-center">-->
    <#--<p class="new-two">碧沙专注于为每一个家庭带来便捷的健康服务</p>-->
    <#--</div>-->
    <#--</div>-->
    <#--<div class="row">-->
    <#--<div class="col-md-8 col-md-offset-2">-->
    <#--<span class="new-there">碧沙科技有限公司是一家致力于大数据、医疗器械研发、制造与销售的高科技企业。2006年由香港宝荣集团（BIS Group）投资创建。原致力于研发石油,油田自动化设备及物联网相关的有源RFID产品。从2014年开始，公司开始着力于大数据医疗器械的研发。2016年成功研制微型3导联动态心电图机并于香港某著名大学大数据中心成功合作设计与实现了心电图机与云计算平台的通讯协议。2017年成功转型集医疗仪器的研发、生产、销售为一体的民营高科技企业。</span>-->
    <#--</div>-->
    <#--</div>-->
    </div>
    <p class="title-p">
        新闻资讯
    </p>
    <div class="line">
        <span class="line-top"></span>
        <span class="line-btm"></span>
    </div>
    <!--置顶新闻部分-->
    <div class="container">
        <div class="row">
            <div class="clear  main_div">
            </div>
        </div>
        <div class="row">
            <div class="col-md-2 col-md-offset-5" style="">
                <a class="text-center cur-p"
                   style="width:85px;height:21px;background:rgba(34,67,150,1);margin-left: 30px;border-radius:10px;display:
                           inline-block;"
                   href="healthInquiry">
                            <span style="color: #FFFFFF;">
                                 更多资讯>
                            </span>
                </a>
            </div>
        </div>
    </div>
    <!--活动优惠-->
    <div class="container-fluid mt-20">
        <div class="row pos-r">
            <img  class="img-responsive center-block" src="../resources/img/index/banner8-cn.gif" alt="碧沙康健心电仪限时活动">
            <a class="text-center taobao">淘宝</a>
            <a class="text-center jingdong">京东</a>
        </div>
    </div>
</div>

<div id="footer"></div>
<script>
    $("#footer").load("footer.html");
</script>
</body>
<script>
    $(document).ready(function() {
        //新闻内容获取
        function indexNew() {

            $.ajax({
                type: "GET",
                dataType: "json",
                url: "/web/call/loadPlacementNews",
                success: function (data) {
                    console.log(data);
                    $.each(data, function(i, item) {
                        var time = getMyDate(item.release_time);
                        $(".main_div").append(
                                '<div class="clear col-md-3 new-div">'+
                                '<div class="row">'+
                                '<input type="hidden"  class="zd-id" value="'+item.id+'"/>'+
                                '<img class=" wh16-9 cur-p news-img" width="280" height="200" src="'+item.img_url+'">'+
                                '<p class="line-h-20 h-20 f-16 col-414141 text-overflow cur-p  text-center mt-10 news-title" style="font-weight:bold;">'+item.news_title+''+
                                '</p>'+
                                '<p class="f-12 col-888 mtb-10-0-ipad h-40 overflow-h line-h-20 text-line-2-1-ipad cur-p hovecol-999 news-describe">'+item.news_describe+''+
                                '</p>'+
                                '<p class="f-12 col-888 mtb-10-0-ipad line-h-20 text-line-2-1-ipad cur-d">'+
                                '<span>时间：</span><span class="col-888 news-time mr-25">'+time+'</span>'+
                                '<span style="margin-left:30px;">阅读量：</span><span class="col-888 news-quantity">'+item.read_quantity+'</span>'+
                                '</p>'+
                                '</div>'+
                                '</div>'
                        )
                    });
                }
            });
        }
        indexNew();
        function getMyDate(data) {
            if (data == "" || data == null) {
                return "- - - - - -";
            }
            data = parseInt(data);
            //var oldTime = (new Date(data)).getTime(); //得到毫秒数
            var oDate = new Date(data);
            oYear = oDate.getFullYear(),
                    oMonth = oDate.getMonth() + 1,
                    oDay = oDate.getDate(),
                    oHour = oDate.getHours(),
                    oMin = oDate.getMinutes(),
                    oSen = oDate.getSeconds(),
                    oTime = oYear + '年' + getzf(oMonth) + '月' + getzf(oDay) + '日';//最后拼接时间
            return oTime;
        };
        function getzf(num) {
            if (parseInt(num) < 10) {
                num = '0' + num;
            }
            return num;
        }
        //点击图片跳转
        $(".main_div").on("click", ".new-div", function () {
            var news_id = $(this).find(".zd-id").val();
            window.location.href = "/web/call/newsContent?news_id=" + news_id;
        });
    });

</script>
</html>
